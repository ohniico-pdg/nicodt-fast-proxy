...
def search():
    ...
    # ══════════════════════════════════════════════════════════
    # SOLUTION C — Court-circuit si artiste bien connu
    # Si validations >= 2 ET mémoire récente → skip Phase 1+2
    # ══════════════════════════════════════════════════════════
    validations   = mem.get("validations", 0)
    derniere_maj  = mem.get("updated_at", "")
    memoire_fraiche = False
    if derniere_maj:
        try:
            diff = datetime.now() - datetime.fromisoformat(derniere_maj)
            memoire_fraiche = diff.days < 7
        except:
            pass

    # Niveau 1 — chemin de recherche mémorisé
    best_path = get_best_path(artiste)

    skip_phases_12 = validations >= 2 and memoire_fraiche and mem.get("site")

    # Si le chemin mémorisé montre que Phase 3 est la plus productive → skip 1+2
    if best_path == "phase3" and validations >= 3:
        skip_phases_12 = True

    if skip_phases_12:
        # Court-circuit — va directement chercher prix + nouvelles dates sur billetterie
        scores         = mem.get("source_scores", {})
        default_src    = ["fnacspectacles.com", "ticketmaster.fr", "infoconcert.com"]
        sorted_sources = sorted(default_src, key=lambda s: scores.get(s, 0.5), reverse=True)
        sites_str      = ", ".join(sorted_sources[:2])
        prompt_cc = f"""{context}

Artiste bien connu — cherche TOUTES les dates disponibles 2025-2026.
Recherche sur ces sites : {sites_str}
Uniquement dates 2025-2026. Si rien : retourne [].
Résultats DÉDUPLIQUÉS. Réponds UNIQUEMENT avec ce JSON : {EVENT_SCHEMA}"""
        try:
            raw_cc = gpt_search(key_gpt, prompt_cc, timeout=80)
            found_cc = extract_json(raw_cc)
            if found_cc:
                events = deduplicate_local(found_cc)
                sources_used = ["court_circuit_billetterie"]
                update_source_scores(artiste, sources_used, True)
                save_cache(artiste, events)
                return jsonify({
                    "results": events,
                    "source": "court_circuit",
                    "sources_used": sources_used,
                    "message": f"Recherche rapide — artiste connu ({validations} validations)"
                })
        except Exception as e:
            print(f"Court-circuit error: {e}", flush=True)
        # Si court-circuit échoue → continue normalement

    # ══════════════════════════════════════════════════════════
    # SOLUTION B — Phases 1 + 2 en PARALLÈLE
    # ══════════════════════════════════════════════════════════

    # Prépare les variantes Instagram
...
Résultats DÉDUPLIQUÉS triés par date. JSON uniquement : {EVENT_SCHEMA}"""
        ...
        # Teste aussi les variantes de site officiel
...
Uniquement dates 2025-2026. JSON uniquement : {EVENT_SCHEMA}"""
    ...
    def run_phase1():
        try:
            raw = gpt_search(key_gpt, prompt1, timeout=50)
            result = extract_json(raw)
            if result and not site_url:
                for ev in result:
    ...
    def run_phase2():
        try:
            raw = gpt_search(key_gpt, prompt2, timeout=45)
            result = extract_json(raw)
            return result, ig_url if result else None
        except Exception as e:
            print(f"Phase 2 error: {e}", flush=True)
            return [], None

    # Exécute Phase 1 et Phase 2 en parallèle
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        future1 = executor.submit(run_phase1)
        future2 = executor.submit(run_phase2)

        try:
            found1 = future1.result(timeout=85)
        except Exception as e:
            print(f"Phase 1 future error: {e}", flush=True)
            found1 = []

        try:
            found2, working_ig = future2.result(timeout=85)
        except Exception as e:
            print(f"Phase 2 future error: {e}", flush=True)
            found2, working_ig = [], None

    if working_ig:
        save_artist_memory(artiste, {"instagram": working_ig})

    events = found1 + found2
    sources_used = []
    if found1: sources_used.append("site_officiel")
    if found2: sources_used.append("instagram")

    # Déduplication locale
    events = deduplicate_local(events)

    # Si >= 2 résultats → STOP, pas besoin de Phase 3
    if len(events) >= 2:
        update_source_scores(artiste, sources_used, True)
        save_cache(artiste, events)
    ...
    # ══════════════════════════════════════════════════════════
    # PHASE 3 — Billetterie (dernier recours uniquement)
    # ══════════════════════════════════════════════════════════
    scores         = mem.get("source_scores", {})
    default_src    = ["fnacspectacles.com", "ticketmaster.fr", "infoconcert.com", "billetreduc.com"]
    sorted_sources = sorted(default_src, key=lambda s: scores.get(s, 0.5), reverse=True)
    sites_str      = "\n".join(f"- {s}" for s in sorted_sources[:3])

    prompt3 = f"""{context}

Recherche les dates de spectacle de "{artiste}" sur :
{sites_str}
Salle exacte, jauge, prix, URL. TOUTES les dates 2025-2026 sans restriction.
Résultats DÉDUPLIQUÉS. JSON uniquement : {EVENT_SCHEMA}"""

    try:
        raw3   = gpt_search(key_gpt, prompt3, timeout=50)
    ...
    if previous:
        merged = merge_results(previous, events)
        save_cache(artiste, merged)
        return jsonify({"results": merged, "sources_used": sources_used})
    save_cache(artiste, events)
    return jsonify({"results": events, "sources_used": sources_used})
...
def validate_event():
    ...
    if "billetterie" in sources or "court_circuit_billetterie" in sources:
